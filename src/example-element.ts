...
export class ExempleElement extends Fetcher(Subscriber(LitElement)) {
  ...
  render() {
    return html`<div>
      <div class="text-xl font-bold">Affichage a l'aide des propriétés déclarées via @property</div>
      <div>${this.first_name} ${this.last_name} : ${this.email}</div>
      <hr class="my-8 opacity-25" />
      <div class="text-xl font-bold">Affichage par dataBinding sans déclaration de proriété</div>
      <div data-bind ::inner-html="$first_name $last_name : $email"></div>
      <hr class="my-8 opacity-25" />
      <div class="text-xl font-bold">Affichage via la propriété props de tout subscriber</div>
      <div>
